...
第二，已经不再需要 Laravel 5.0 中使用的 `App\Services\Registrar` 类。你可以简单的从这个类直接复制你的 `validator` 和 `create` 方法粘贴至你的 `AuthController`。这些方法不需要做其他修改。然而，你必须确定有在你的 `AuthController` 顶端引入 `Validator` facade 跟你的 `User` 模型。
...
复制项目内任何其他的文件，例如： `.scrutinizer.yml`, `bower.json` 以及其他类似的工具配置文件。
