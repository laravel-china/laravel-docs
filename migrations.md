...
Laravel 的 `Schema` [facade](/docs/{{version}}/facades) 对数据表的创建和操作提供了相应支持。它在所有 Laravel 支持的数据库系统中共用了一套相同的 API。

<a name="generating-migrations"></a>
## 生成迁移

你可以使用 `make:migration` [Artisan 命令](/docs/{{version}}/artisan) 来创建迁移：
...
你可以在这两个方法中使用 Laravel 结构构造器来创建及修改数据表。若要了解`结构`构造器中的所有可用方法，[可查阅它的文档](#creating-tables)。让我们来创建一张 `flights` 数据表作为示例：
...
要运行应用程序中的所有未执行迁移，可以使用 `migrate` Artisan 命令。如果你使用了 [Homestead 虚拟主机](/docs/{{version}}/homestead)，则应该在虚拟机中运行如下命令：
...
当然，当创建数据表时，你也可以使用任何结构构造器的 [字段方法](#creating-columns) 来定义数据表的字段。
...
若要更新一张已存在的数据表，我们会使用 `Schema` facade 的 `table` 方法。如同 `create` 方法，`table` 方法会接收两个参数：一个是数据表的名称，另一个则是接收 `Blueprint` 实例的`闭包`。我们可以使用它来为数据表新增字段：
...
> **注意：**数据表的 `enum`，`json` 或者 `jsonb`字段暂时不支持修改字段属性。
...
> **注意：**数据表的 `enum`，`json` 或者 `jsonb`字段暂时不支持修改字段名称。
