...
一个「一对多」关联使用于定义单个模型拥有任意数量的其他关联模型。例如，一篇博客文章可能有无限多个评论。就像其他的 Eloquent 关联一样，可以借由放置一个函数至你的 Eloquent 模型来定义一对多关联：
...
多态关联允许一个模型在单个的关联从属一个以上的其他模型。举个例子，假设你想保存照片给你的工作人员及你的产品。使用多态关联，你可以对这两种情况使用单个的 `photos` 数据表。首先，让我们查看创建这种关联所需的数据表结构：
