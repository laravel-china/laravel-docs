...
一个「一对多」关联使用于定义单个模型拥有任意数量的其它关联模型。例如，一篇博客文章可能有无限多个评论。就像其它的 Eloquent 关联一样，可以借由放置一个函数至你的 Eloquent 模型来定义一对多关联：
...
当然，就像所有的其它关联类型，你可以调用 `roles` 方法，接着在该关联之后链式调用上查找的条件：
...
默认情况下，`pivot` 对象只提供模型的键。如果你的 pivot 数据表包含了其它的属性，可以在定义关联方法时指定那些字段：
...
多态关联允许一个模型在单个的关联从属一个以上的其它模型。举个例子，假设你想保存照片给你的工作人员及你的产品。使用多态关联，你可以对这两种情况使用单个的 `photos` 数据表。首先，让我们查看创建这种关联所需的数据表结构：
...
除了一般的多态关联，你也可以定义「多对多」的多态关联。例如，博客的 `Post` 和 `Video` 模型可以共用多态关联至 `Tag` 模型。使用多对多的多态关联能够让你的博客文章及影片共用独立标签的单个列表。首先，让我们查看数据表结构：
...
在这个例子里，Eloquent 只会预加载文章标题字段包含 `first` 的文章。当然，你也可以调用其它的[查询语句构造器](/docs/{{version}}/queries)来进一步自定预加载的操作：
