...
你可能想要将扩展包的配置文件发布到应用程序本身的 `config` 目录。这能够让扩展包的用户轻松重写这些默认的设置选项。如果要发布扩展包的配置文件，只需要在服务提供者里的 `boot` 方法里使用 `publishes` 方法：
...
现在当扩展包的用户使用 Laravel 的 `vendor:publish` 命令时，扩展包的文件将会被复制到指定的位置上。当然，只要你的配置文件被发布，就可以如其他配置文件一样被访问：
...
你的扩展包内可能会包含许多的资源文件，像是 JavaScript、CSS 和图片。如果要发布这些资源文件到应用程序的 `public` 目录上，只需使用服务提供者的 `publishes` 方法。在这个例子中，我们也会增加一个 `public` 的资源分类标签，可被使用于发布与分类关联的资源文件：
...
现在当扩展包的用户运行 `vendor:publish` 命令时，资源文件将会被复制到指定的位置。每次扩展包需要更新重写资源文件时，可以使用 `--force` 来标记：
...
你可能想要分别发布分类的扩展包资源文件或是资源。举例来说，你可能想让用户不需发布扩展包的所有资源文件，只需单独发布扩展包的配置文件。可以通过在调用 `publishes` 方法时使用「标签」来实现。例如，让我们在扩展包的服务提供者中的 `boot` 方法定义两个发布群组：
