...
除了在单一的 `routes.php` 文件中定义所有的请求处理逻辑，您可能希望使用控制器类来组织此行为。控制器可将相关的 HTTP 请求处理逻辑组成一个类。控制器一般存放在 `app/Http/Controllers` 目录下。
...
若你选择在 `App\Http\Controllers` 目录内层，使用 PHP 命名空间嵌套或组织控制器，只要使用相对于 `App\Http\Controllers` 根命名空间的特定类名称即可。因此，若您的控制器类全名为 `App\Http\Controllers\Photos\AdminController`，您可以像这样注册一个路由：

    Route::get('foo', 'Photos\AdminController@method');

#### 命名控制器路由

就像闭包路由，您可以指定控制器路由的名称：

    Route::get('foo', ['uses' => 'FooController@method', 'as' => 'name']);

#### 控制器行为的 URLs

您也可以使用 `route` 辅助方法，产生命名控制器路由的 URL：
...
资源控制器让您可以无痛地创建与资源有关的 RESTful 控制器。例如，你可能想要创建一个控制器，用来处理对你应用程序保存「相片」发送的 HTTP 请求。使用 `make:controller` Artisan 命令，我们可以快速地创建像这样的控制器：
...
这一条路由声明会创建多个路由，用来处理各式各样和相片资源相关的的 RESTful 行为。同样地，产生的控制器已有各种和这些行为绑定的方法，并包含通知您它们所处理的 URI 及动词的记录。
...
有时您可能会需要对「嵌套」资源定义路由。例如，相片资源可能会附带多个「注解」。要「嵌套化」此资源控制器，可在路由声明中使用「点」记号：

    Route::resource('photos.comments', 'PhotoCommentController');

此路由会注册一个「嵌套化」的资源，可透过下面这样的 URL 来访问它：`photos/{photos}/comments/{comments}`。
...
若您必须在资源控制器中默认的资源路由之外，加入额外的路由，您应该在调用 `Route::resource` 之前定义这些路由。否则，由 `resource` 方法定义的路由可能会意外地覆盖您附加的路由：
...
Laravel 让您能够轻易地透过定义单一路由来处理控制器类中的各种行为。首先，使用 `Route::controller` 方法来定义路由。`controller` 方法接受两个参数。第一个参数是控制器所处理的基底 URI，第二个是控制器的类名称：
...
正如您在上述例子中所看到的，`index` 方法会回应控制器所处理的根 URI，在这个例子中是 `users`。
...
当然，您也可以对任何的 [Laravel contract](/docs/{{version}}/contracts) 使用类型提示。若容器能够解析它，你就可以使用类型提示。
...
你依然可以做 `Illuminate\Http\Request` 类型提示并透过定义你的控制器方法类似下面例子，来访问你的路由参数 `id`：
...
若您的应用程序完全透过控制器使用路由，您可以利用 Laravel 的路由缓存。使用路由缓存可以大幅降低注册您应用程序全部的路由所需的时间。在某些情况下，您的路由注册甚至可以快上一百倍！要产生路由缓存，只要运行 `route:cache` 此 Artisan 命令：

    php artisan route:cache

这就是全部了！现在你的缓存路由文件将被用来代替 `app/Http/routes.php` 此一文件。请记得，若你添加了任何新的路由，就必须产生新的路由缓存。因此你可能希望只在您的项目部署时才运行 `route:cache` 此一命令。
