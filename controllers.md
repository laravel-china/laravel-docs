...
- [资源控制器](#resource-controllers)
    ...
    - [本地化资源URI](#restful-localizing-resource-uris)
...
[中间件](/docs/{{version}}/middleware) 可以在路由文件中指定给控制器的路由：

    Route::get('profile', 'UserController@show')->middleware('auth');

然后，在控制器的构造方法中指定中间件会更为便捷。在控制器的构造方法中使用 `middleware` 方法，你可以很容易地将中间件指定给控制器操作。你甚至可以约束中间件只对控制器类中的某个特定方法生效：
...
控制器也允许你使用闭包的方式注册中间件。通过这个方法，你可以快捷的为单个控制器定义一个中间件，而不需要定义一个完整的中间件类：
...
> {tip} 你可能将中间件指定到控制器的部分操作上，然而，这样会使得你的控制器过于臃肿。换个角度，你可以考虑将控制器分成多个更小的控制器。
