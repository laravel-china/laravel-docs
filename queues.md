...
要推送任务到不同的队列上，你要将任务先「分类」，甚至可能要排定每个队列能有多少作业器可以运行任务。这并不是指任务会推送到你在配置文件所定义的不同队列链接里，而是推送到某个有单个链接的队列。要指定任务运行的队列，可以用任务实例的 `onQueue` 方法。`onQueue` 是 `Illuminate\Bus\Queueable` trait 所提供的方法，而它已经包含在 `App\Jobs\Job` 基类中：
